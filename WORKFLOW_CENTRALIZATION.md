...
- `.github/actions/benchmark-pages`: Syncs the shared benchmark page (the `index.html` colocated with the action) into a repo's gh-pages branch, writes a marker-guarded root redirect to `benchmarks/`, and, for multi-folder layouts like storage, regenerates `folders.json` and replaces the per-package stock pages with redirect stubs that preselect the package filter. Runs automatically as the last step of the `benchmark.yml` workflow (disable with `sync-pages: false`) or standalone as a post-matrix job. Because the page is copied on every publish, edits to the central `index.html` roll out to all repos with their next benchmark run.
