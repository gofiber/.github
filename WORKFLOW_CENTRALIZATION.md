...
- `pr-command-hint.yml`: Companion to `pr-command.yml`. On `pull_request_target: opened` it posts one small comment naming the repository's commands, skipping pull requests opened by bots. One call per repository rather than per command, so `gofiber/utils` with two commands still gets a single comment. It checks out nothing and passes no pull request data to a script, which is why the write token of `pull_request_target` is safe here.
