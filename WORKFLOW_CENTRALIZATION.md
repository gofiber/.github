...
- `issue-auto-reply.yml`: Answers the two replies maintainers write over and over. On `labeled` it posts `.github/issue-replies/<slug>.md`, where the slug is the label name reduced to ascii (`♻ Wait for Response` becomes `wait-for-response`), and does nothing when no such file exists; a hidden marker in the comment keeps a re-applied label from repeating it. On `opened` it reads the version field of the issue form and asks the reporter to retry only when a newer release of the *same major* exists, so a v2 report is never answered with a v3 release.
